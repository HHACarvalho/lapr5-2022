...
%carateristicasCam(?Nome, ?Tara, ?CapCarga, ?CapBateria, ?Autonomia, ?TpsRecarregamento)
...
%dadosCam_t_e_ta(?NomeCamiao, ?CidadeAID, ?CidadeBID, ?Tempo, ?Energia, ?TempoAdd)
...
%entrega(?ID, ?Data, ?Massa, ?ArmazemID, ?TempoColoc, ?TempoRet)
...
algoritmo_genetico_varios_camioes(ListaCamioes, ListaEntregas, TempoIdeal,Data):-
	calculo_carga_total_transportar(ListaEntregas, MassaTotalTransportar),
	calculo_Capacidade_Total_Camioes(ListaCamioes, CapacidadeTotalCamiao),
	MassaTotalTransportar =< CapacidadeTotalCamiao,
	verifica_numero_camioes(ListaCamioes, MassaTotalTransportar),
	!,
	(retract(tempoIdeal(_)), !; true), asserta(tempoIdeal(TempoIdeal)),
	(retract(dataEntrega(_)), !; true), asserta(dataEntrega(Data)),
	get_time(InitTempoExecucao),
	...
	gera_populacao(Pop, ListaArmazem),
	write('Pop: '),write(Pop),nl,
	avalia_populacao(ListaCamioes, Pop, PopAv),
	write('PopAval: '),write(PopAv),nl,
	ordena_populacao(PopAv,PopOrd),
	geracoes(NG),
	gera_geracao(ListaCamioes,0,NG,PopOrd,[PopOrd]).
